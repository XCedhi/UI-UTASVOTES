...
const ManageElectionInteractive = ({ electionId }: ManageElectionInteractiveProps) => {
  ...
  const handleTogglePosition = (id: string) => {
    setPositions(
      positions.map((p) =>
        p.id === id ? { ...p, status: p.status === 'open' ? ('closed' as const) : ('open' as const) } : p
      )
    );
  };
                  ...
                  <div className="space-y-4 pt-4 border-t border-border">
                    ...
                    <label className="flex items-center gap-3 cursor-pointer">
                      <input
                        ...
                        onChange={(e) =>
                          setElectionData({ ...electionData, requireVerification: e.target.checked })
                        }
                        className="w-5 h-5 rounded border-border text-primary focus:ring-2 focus:ring-primary"
                      />
                      <div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {electionData.status === 'active' && (
                      ...
                      >
                        <Icon name="PauseIcon" size={24} variant="outline" className="text-warning" />
                    ...
                    {electionData.status === 'paused' && (
                      ...
                      >
                        <Icon name="PlayIcon" size={24} variant="outline" className="text-success" />
                        <div className="flex-1 text-left">
